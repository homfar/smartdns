...
func TestMergeDeltaUpdatesVersion(t *testing.T) {
	...
	var v int
	_ = d.QueryRow(`SELECT version FROM zones WHERE domain='example.com'`).Scan(&v)
	if v < 2 {
		t.Fatalf("expected version increment, got %d", v)
	}
}
